...
if (isset($_POST['submit'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        // Update the admins table with OTP, token, and expiry
        $update_stmt = $dbconnection->prepare("UPDATE admins SET otp = ?, otp_expiry = ?, verification_token = ? WHERE email = ?");
        $update_stmt->bind_param("ssss", $otp_hash, $otp_expiry, $verification_token, $email);
        
        if ($update_stmt->execute()) {
            // Create verification link
            $verification_link = "https://madridejosbh2.com/admin/verify_otp.php?token=" . $verification_token;
